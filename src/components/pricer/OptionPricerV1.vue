...
export default {
  ...
  methods: {
    sendToServerForCalc(optData, col) {
      if (this.totalsToggle) {
        this.sendToServerForCalcTotals();
      } else {
        this.sendToServerForCalcSingle(optData, col);
      }
    },
    async sendToServerForCalcTotals() {
    ...
    formatTotalsColumn(cellName, keyGroup) {
      ...
      const backgroundColor = this.shadeColor(keyGroup.BackgroundColor, -10);
      if (
  ...
  watch: {
    ...
    totalsToggle() {
      ...
      let areCrossesTheSame = rowData.every((e) => e === rowData[0]);
      console.log(areCrossesTheSame);
