<template>
  <div class="ml-5">
    <!-- <v-btn @click="dev" color="yellow">DEV</v-btn> -->
    <div
      ...
      <div class="d-flex flex-column mr-1">
        <div class="d-flex flex-row">
          <div class="d-flex flex-column mr-1">
            ...
            <div>
              <ZSCoreScannerChartScatter
                :key="componentKey"
                :labels="uniqueCrosses"
                :scatter_data="scatter_data"
                :chartTitle="`${chart_title}`"
              />
              <ZScoreScannerChartBar
                class="mt-10"
                :key="componentKey"
                :labels="uniqueCrosses"
...
import ZScoreScannerChartBar from "@/components/VolAnalytics/Scanners/RealizedZScoreScanner/ZScoreScannerChartBar.vue";
import ZSCoreScannerChartScatter from "@/components/VolAnalytics/Scanners/RealizedZScoreScanner/ZScoreScannerChartScatter.vue";
  ...
  methods: {
    ...
    createBarChartData(inputArray, term) {
      ...
      for (const item of this.uniqueCrosses) {
        ...
        if (index > -1) {
          arr.push(filteredArr[index].ZScore);
        } else {
          arr.push(0);
        }
      }
    ...
    async getApiData() {
      try {
        let response = await VolAnalyticsApi.get_realized_zscore_scanner(
          this.volEstName,
